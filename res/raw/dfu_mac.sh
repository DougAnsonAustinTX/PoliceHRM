...
usage() {
    ...
    echo "      -m Type of the hex file. May be: a, b or s, where a=application, b=bootloader, s=soft device."
...
# Start DFU Initiator activity if no target device specified
    ...
    adb $S_DEVICE shell am start -a no.nordicsemi.android.action.DFU_UPLOAD --ei no.nordicsemi.android.dfu.extra.EXTRA_FILE_TYPE $MODE -e no.nordicsemi.android.dfu.extra.EXTRA_FILE_PATH "/sdcard/Nordic Semiconductor/$HEX_FILE" | grep "unable" > /dev/null 2>&1
    if [ "$?" = "0" ] ; then
        echo "FAIL"
