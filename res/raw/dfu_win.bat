...
rem Description:
rem ------------
rem The script allows to perform DFU operation from the computer using an Android device as a dongle.
...
rem Read the optional upload mode. The default mode is "application" or "a".
if /I "%~1"=="-m" set MODE_SET=1
if defined MODE_SET (
	if not "%~1"=="" (
		if "%~2"=="s" set MODE=1
		if "%~2"=="b" set MODE=2
...
if "%ADDRESS%"=="" (
 ...
 --ei no.nordicsemi.android.dfu.extra.EXTRA_FILE_TYPE %MODE%^
...
:usage_only
echo Usage:
echo %PROGRAM% [-D device_id] [-A address] [-N name] [-M mode] firmware.[hex or zip]
echo Info:
...
echo mode      - Type of the hex file. May be: a, b or s, where a=application, b=bootloader, s=soft device.
echo             The mode will be automatically obtained from ZIP file. Files in the ZIP file MUST have the following names:
echo             application.hex - the application
echo             softdevice.hex - the new soft device
echo             bootloader.hex - the new bootloader
echo             ZIP file must contain one, two or all of those and must not contain other files.
goto exit
